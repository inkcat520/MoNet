...
from data_gen.cmu_mocap import cmu
...
from utils.batch_sample import get_batch_srnn_cmu
...
def run():
    ...
    # data loading
    dataset = cmu(data_dir=args.data)
    ...
    if args.mode == 'train':
        ...
        for act in dataset.define_acts(args.act):
            test_gen[act] = get_batch_srnn_cmu(test_set, act, args.seq_len, args.pred_len, args.key_point, args.manner)

        print(">>> loading train data")
        train_set = dataset.load_data(dataset.train_subject, args.act)
        train_gen = TrainData(train_set, args.seq_len, args.pred_len, sample_start=0, data_flip=args.aug)
        loader_train = DataLoader(train_gen, batch_size=args.batch_size, shuffle=True, drop_last=True, num_workers=0, pin_memory=True)
        work_dir, log = runtime.exp_create(args.exp, cmu.exp, args.cfg)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if epoch % args.eval_interval == 0:
                ...
                save_ckpt(work_dir, cmu.exp, state, epoch, avg_avg_angle, args.keep)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data', type=str, default='./data/cmu', help='path to cmu dataset')
    parser.add_argument('--seed', type=int, default=1234567890, help='random seed')
    parser.add_argument('--cfg', type=str, default='cfg/cmu.yml', help='path to the configuration in .yml')
    ...
    parser.add_argument('--manner', type=str, default='80', help='all or 256 or 80')
    ...
    parser.add_argument('--subact', type=str, default=1, help='eval subaction')
